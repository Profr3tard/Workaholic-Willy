...
#: ``WILLY_PROFILE`` accepts a COMMA-SEPARATED CHAIN of profile layers
_PROFILE_SEPARATOR = ","

#: When set to a readable YAML path, the loader deep-merges that overlay into the ``robot``
#: section of the raw config **before** Pydantic validation. 
_ADAPTATION_OVERLAY_ENV_VAR = "WILLY_ADAPTATION_OVERLAY"

#: Reset sentinel for PROFILE overlays.
...
def _describe_validation_error(exc: ValidationError, root: Path, profile: str) -> str:
    ...
    section, left the reader guessing which of them carried the typo.
    ...
    for err in exc.errors():
        ...
        if origin is not None:
            ...
            # No origin means the key is NOT in any YAML.
...
def _load_models_section(root: Path, profile: str) -> dict[str, Any]:
    ...
    Duplicate keys across files are rejected.
    """
...
def _resolve_overlay_resets(value: Any) -> Any:
    ...
    becomes ``None`` in the final config.
    """

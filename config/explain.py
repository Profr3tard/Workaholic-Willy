...
__all__ = ["KeyExplanation", "Layer", "decisions", "explain", "explain_key", "find_keys"]
...
class KeyExplanation:
    ...
    required: bool = False
    doc: str = ""
    #: ``"means"`` for per-field prose, ``"block"`` when it fell back to the block's docstring.
    doc_scope: str = ""
    layers: tuple[Layer, ...] = ()
...
def explain_key(path: str, root: Path, layers: tuple[str, ...], value: Any = None) -> str:
    ...
    return explain(path, root, layers, value).render()

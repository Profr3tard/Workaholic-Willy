...
def main(argv: list[str] | None = None) -> int:
    ...
    # `config --profile X explain KEY` and `config explain KEY --profile X` both read naturally.
    ...
    if args.command == "where":  # schema-only
        from .explain import find_keys

        # `--tier` and `--limit` were parsed and then dropped on the floor here, so the flag the help
        # text advertised did nothing and every listing silently stopped at 40 keys per tier.
    ...
    if args.print:
        ...
        # Report the chain that was actually LOADED.
        chain = " -> ".join(

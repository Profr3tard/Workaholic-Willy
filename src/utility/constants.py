...
#: these helpers share nothing but a package. A "why is this running on CPU"
#: question and a "where did my debug PNG go" question are never read in the
#: same sitting, so there is no chronological narrative for an aggregate
#: ``utility.log`` to preserve.
...
def utility_logger(name: str, log_file: str, level: int = logging.INFO) -> Logger:
    ...
    A lazy accessor rather than the usual module-scope
    ``logger = create_logger(...)``. ``utility.paths`` is imported by this
    package's ``__init__`` and therefore by nearly every process here,
    including ones that only resolve a path and never reach a line worth
    logging. Building the logger on first use is what keeps the package free of
    import-time global state: no log directory is created, and no permanently
    empty ``paths.log`` appears, until something has something to say.
    ``lru_cache`` keeps the call sites cheap enough to write inline.
    """

...
----------------------------------------------
    ...
    This is exactly what :class:`EyeInHandCalibrator` builds
    (``eye_hand/eye_in_hand/calibrator.py:22-28``); only ``B`` is a plain output of
    :meth:`relative_motions`, so ``A`` must be built manually.

    .. warning::
       This block previously documented the pair the other way round -- marker on the ``A``
       side, gripper on the ``B`` side, with both index orders reversed. That pair is the
       **exact inverse** of this one (if ``A' = inv(B)`` and ``B' = inv(A)``, then
       ``A' X' = X' B'`` is solved by ``X' = inv(X)``), so hand-rolling a solve from it
       returned ``T_tool_to_cam`` while every label said ``T_cam_to_tool``: a silently
       inverted transform, roughly 290 mm out on the shipped sim geometry, with no error
       raised anywhere. The CODE was always right -- the eye-in-hand pick lands 0.1 mm
       on-target on-box -- so only this docstring changed.
...
class HandEyeAXXB:
    ...
    def _solve_rotation(
        ...
        for i, (A, B) in enumerate(zip(A_mats, B_mats)):
            R_A = A[:3, :3]
            R_B = B[:3, :3]
            # Column-major formula: (I⊗R_A) vec(X) = vec(R_A X),
            #                       (R_B^T⊗I) vec(X) = vec(X R_B).
            K[9 * i : 9 * (i + 1)] = (

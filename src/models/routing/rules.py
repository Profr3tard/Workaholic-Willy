...
WHY RULES AND NOT A CLASSIFIER. The router runs before any weights load, on every pick, and its
mistakes are expensive in both directions: a false SIMPLE grasps the wrong object, a false VLM costs
seconds and VRAM.
"""
...
#: red cube", but "small red cube on the left of the big blue box" is a different problem.
MULTI_ATTRIBUTE_THRESHOLD = 3

# --- vocabularies ---------------------------------------------------------------------------------
# Tokens that are unambiguously German. Ambiguous ones are EXCLUDED on purpose: "die" is a German
# article and an English verb, "in"/"an"/"so"/"war" collide too.
_GERMAN_MARKERS = frozenset({
    ...
    # NB "war", "hat" and "leg" are German words AND ordinary English ones, they are deliberately
    # absent. A test ("the war memorial") caught all three; a false NON_ENGLISH silently doubles the
    # cost of every English pick, so this set stays strictly free of English readings.
...
_COMPARATIVE = frozenset({
    ...
    # "next" is absent: on its own it is an ordinal, but "next TO" is spatial, and the bigram is by far
    # the commoner phrasing at a bench. It is handled in _SPATIAL_PHRASES instead.

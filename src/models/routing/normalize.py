...
Normalising is string handling and not a language model, so it cannot silently change which object was
asked for.
...
#: Sentence-final marks that already terminate a phrase. A prompt ending in one keeps it rather than
#: acquiring a second terminator.
_TERMINATORS = (".", "?", "!")
...
def normalize_simple_prompt(prompt: str) -> str:
    ...
    caption with one empty phrase, which grounds nothing and reads like a real prompt in a log.
    """

...
# Normalised labels that map onto the four values this package commits to.
...
# Declartion of mediapipes none/unknown labels, so we can map them to HandGesture.NONE.
...
class ThumbGestureRecognizer:
    ...
    def _read_gesture(self, categories: Any, hand_index: int) -> GestureReading:
        ...
        Below `min_gesture_confidence` the answer is `NONE`, not a low-confidence `THUMB_UP`
        """
        if not categories:

...
class RtDetrObjectDetector:
    ...
    def _class_filter(self, prompt: str | None) -> str | None:
        ...
        RT-DETR is CLOSED-SET: it has no text encoder, and the prompt never reaches the model. It can
        only be matched against the trained class list afterwards.
        """
    ...
    def detect_all(self, image: np.ndarray, prompt: str | None = None) -> list[Detection]:
        ...
        Raises ``ValueError`` when ``prompt`` is free text rather than a class name: see
        :meth:`_class_filter` for why that is louder than returning nothing.
        """

...
from config.schema.models.models_schema import ObjectDetectorConfig
...
class GroundingDinoObjectDetector:
    ...
    def __init__(self, config: ObjectDetectorConfig, debug_images: bool = False):
        ...
        if self.local and not Path(source).is_dir():
            raise FileNotFoundError(
                ...
                f"mode, either put the model files there, or set local: false "
                f"and models.objectdetector.model_id to a Hub id "
    ...
    def detect(self, image: np.ndarray, prompt: str) -> Detection:
        ...
        # Rescale box coordinates from normalized [0,1] to pixel values
    ...
    def detect_all(self, image: np.ndarray, prompt: str) -> list[Detection]:
        """Like :meth:`detect` but returns ALL detections above threshold.
        ...
        if not results or len(results[0]["scores"]) == 0:
            # Logged too: "grounded nothing" is a result with a cost, and leaving it silent is how a
            # slow, empty perception loop looks like a fast one.

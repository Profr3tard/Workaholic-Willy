...
----------------------------------------------
    A_i  = T_cam_to_marker_i @ inv(T_cam_to_marker_{i+1})   (backward relative marker motion)
    B_i  = inv(T_base_to_tool_i) @ T_base_to_tool_{i+1}     (local relative gripper motion)
    X    = T_cam_to_tool                                      (solve target)

    Note: A is the **inverse** of the standard relative camera motion and B is
    the **left** (local-frame) relative EE motion — neither is a plain output
    of :meth:`relative_motions`.  The caller must build these manually.

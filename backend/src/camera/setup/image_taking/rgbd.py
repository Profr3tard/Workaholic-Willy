...
so :class:`~backend.src.camera.orchestration.frame_provider.FrameProvider` can select
either via ``RGBDDeviceRigConfig.rgbd_backend`` without the rest of the pipeline caring.

* :class:`OpenCvRGBDStreamer` — generic ``cv2.VideoCapture`` + OpenNI depth channel.
...
from backend.config.schema.camera import RGBDDeviceRigConfig
from backend.src.camera.setup.image_taking.frames import RGBDFrame
from backend.src.camera.setup.quality import configure_camera_for_quality
...
class OpenCvRGBDStreamer:
    ...
    def grab(self) -> RGBDFrame:
        ...
        # Depth channel — may not be available for all backends/devices
...
class RealSenseRGBDStreamer:
    ...
    def _match_color_size(depth: np.ndarray, color: np.ndarray) -> np.ndarray:
        ...
        un-aligned native depth resolution) — nearest keeps depth values intact.
        """
    ...
    # ------------------------------------------------------------------
    # Internal — device / filter configuration
    # ------------------------------------------------------------------
    ...
    def _apply_visual_preset(self, rs: Any, sensor: Any, preset: str) -> None:
        ...
        rather than raised — a bad preset name must not take the whole stream down.
        """
    ...
    def _read_intrinsics(self, rs: Any, profile: Any) -> np.ndarray | None:
        """Read the colour-stream 3x3 K matrix from the active profile."""
        try:
    ...
    def _export_intrinsics(self, k: np.ndarray) -> None:
        ...
        from backend.src.utility import dump_json, ensure_dir
        ...
        self.logger.info("Exported RealSense colour intrinsics to %s", path)

...
from backend.config.schema.models.models_schema import SegmenterConfig
from backend.src.models._inference import (
    autocast_ctx,
    build_load_kwargs,
    finalize_model,
)
from backend.src.models.constants import MODELS_LOG_DIR, SEGMENTER_LOG_FILE
...
class Sam2Segmenter:
    ...
    def segment(
        ...
        """
        Made SAM2 for exact one bounding box.

        Parameter:
        - image_rgb: HxWx3 RGB numpy array
        - bbox_xyxy: (x1, y1, x2, y2)
        - label: Objektlabel
        - score: optionaler vorgelagerter Detection-Score
        """
    ...
    def _extract_first_mask(self, post_masks: Any) -> np.ndarray:
        """
        Extracts the first 2D mask out of the processor
        """
        ...
        # If we have list/tuple, always choose first element
    ...
    def _postprocess_mask(self, raw_mask: np.ndarray) -> np.ndarray:
        ...
        - optional: keep biggest component
        """

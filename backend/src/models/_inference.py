...
from backend.config.schema.models.models_schema import InferenceOptimization
from backend.src.utility.device import resolve_torch_dtype
...
def build_load_kwargs(
    ...
    if optim.torch_dtype is not None:
        ...
        if resolved == torch.float16:
            # fp16 drops small-object detection recall (the M2 lesson). The DEFAULT (torch_dtype=None)
            # keeps HF fp32 weights; this fires only when a config explicitly asks for "auto"/fp16.
            _log.warning(

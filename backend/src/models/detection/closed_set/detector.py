...
from backend.src.models.constants import MODELS_LOG_DIR, RTDETR_LOG_FILE
...
class RtDetrObjectDetector:
    ...
    optional ``prompt`` -- passed positionally by the perception seam -- is used
    only as a case-insensitive class-name filter; ``None`` returns every class
    above threshold.
    """
    ...
    def detect_all(self, image: np.ndarray, prompt: str | None = None) -> list[Detection]:
        ...
        if not results:
            return []
        wanted = prompt.strip().lower() if prompt and prompt.strip() else None
        dets: list[Detection] = []

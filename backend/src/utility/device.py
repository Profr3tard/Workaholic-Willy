...
Production runs on CUDA whenever a GPU is present, while tests and
developer machines fall back gracefully to CPU (or MPS on Apple
silicon).  The selection can be forced via the ``WILLY_DEVICE``
...
def resolve_torch_dtype(
    ...
    On CUDA, ``None``/``auto`` resolves to ``float16`` (broadly supported,
    fastest path on consumer GPUs).  On CPU/MPS it resolves to ``float32``
    because half-precision matmul on those backends is usually slower or
    unsupported.
    """
    ...
    if dtype != torch.float32 and device.type != "cuda":
        # Half precision on CPU/MPS is rarely a win and often errors out.

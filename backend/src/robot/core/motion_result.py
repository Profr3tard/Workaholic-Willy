...
:meth:`RobotArm.move`. The bool-returning methods remain as
compatibility shims and are being migrated onto this typed surface.
...
returned as a typed :class:`MotionResult`, **not** raised. Drivers MAY
raise on connection/transport faults if they prefer the exception path,
but callers should be able to rely on a typed result for everything
that is not a true runtime fault. The :meth:`MotionResult.from_bool`
and :meth:`MotionResult.executed` helpers exist so driver authors do
not need to hand-roll the dataclass for every code path.
"""
...
class MotionStatus(StrEnum):
    ...
    # :class:`backend.src.robot.safety.SafetyPreflight` pipeline. Drivers
...
class MotionResult:
    ...
    accidental mutation. Construct via :meth:`executed`,
    :meth:`failed`, or :meth:`from_bool` rather than the bare
    constructor for clarity at call sites.
    """
    ...
    def from_bool(
        ...
        *,
        ...
        ``failure_status`` is the category to assign when ``ok`` is
        ``False``. The default is :attr:`MotionStatus.CONTROLLER_REJECTED`
        because that is the most common cause for drivers that
        returned ``False`` without a richer classification. Callers
        that have already proved a more specific cause (e.g.
        ``WORKSPACE_REJECTED`` from a pre-flight check) should set
        ``failure_status`` explicitly.
        """
        if ok:
            return cls.executed(
                command,

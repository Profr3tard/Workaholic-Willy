...
:mod:`backend.src.robot.grippers` registers a factory under one of
these enum members. Pipelines never read this enum; they go through
the :class:`~backend.src.robot.core.Gripper` Protocol.

Adding a new gripper is a two-line change here PLUS a driver module
under ``backend/src/robot/grippers/<name>.py`` (or subpackage).
"""
...
class GripperVendor(StrEnum):
    ...
    NONE
        Explicit "no gripper attached" --- a no-op implementation that
        still satisfies the :class:`Gripper` Protocol so pipelines can
        stay branch-free.
    """

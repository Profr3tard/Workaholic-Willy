...
* :class:`backend.config.schema.robot.RobotConfig` (operators select a driver via the
  ``vendor`` field — validated against THIS enum at config-load).
* :mod:`backend.src.robot.drivers.registry` (driver factories register under one of
  these identifiers).
* :class:`backend.src.robot.core.RobotCapabilities` — its ``vendor`` field is deliberately
...
package under ``backend/src/robot/drivers/<name>/``. Pipelines never
read this enum directly --- they go through the
:class:`~backend.src.robot.core.RobotArm` Protocol.
"""

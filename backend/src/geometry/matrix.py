...
1. Bridging legacy code (e.g. eye-to-hand calibration, OpenCV) that
   speaks plain ndarrays — see also
   :mod:`backend.src.geometry.conversions`.
2. Numerical efficiency for tight inner loops where a single matrix
   multiply is cheaper than allocating a new :class:`Transform`.

For *all other purposes*, prefer :class:`~backend.src.geometry.transform.Transform`,
which carries explicit frames and is type-checked at every step.
"""
...
def make_homogeneous(R: np.ndarray, t: np.ndarray) -> np.ndarray:
    """Assemble a 4x4 transform from a 3x3 rotation and (3,) translation."""
...
def position_quaternion_to_matrix(t: np.ndarray, q: np.ndarray) -> np.ndarray:
    """Assemble a 4x4 transform from a (3,) translation and XYZW quaternion."""
...
def invert_homogeneous(T: np.ndarray) -> np.ndarray:
    """Return the inverse of a 4x4 rigid transform (analytical, no solver)."""
    arr = validate_homogeneous_matrix(T)
    R = arr[:3, :3]
    t = arr[:3, 3]

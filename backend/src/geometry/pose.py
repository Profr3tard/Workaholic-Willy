...
* ``frame``              — :class:`~backend.src.geometry.frame.Frame`
...
class Pose:
    ...
    * the quaternion is normalised and put into canonical sign form
      (``w >= 0``);
    * both ndarrays are made read-only.

    Equality and hashing use byte-wise array comparison after
    canonicalisation, which is well defined because of the sign
    convention.
    """
    ...
    def with_frame(self, frame: Frame) -> Pose:
        ...
        This does **not** transform the pose — it just relabels it. Use
        :meth:`Transform.apply_pose` for actual coordinate changes.
        """
        return Pose(

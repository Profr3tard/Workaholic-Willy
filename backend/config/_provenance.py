...
user actually has — *which layer set this?* and *why this number?* — and neither can its error messages.
...
def section_sources(root: Path) -> list[tuple[Path, str, str]]:
    ...
    two can be compared by eye — and a test asserts every indexed key resolves to a real loaded value.
    """
    return [

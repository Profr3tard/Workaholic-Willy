...
resolves another robot's link lengths in every one of the lookups above — silently, and on real
hardware that is not a cosmetic error.
...
``backend.src.robot.drivers.sim.robot_models._UR_MODELS`` by ``tests/test_sim_robot_models.py``.
"""

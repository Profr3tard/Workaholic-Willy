...
class GraspingClosedLoopConfig(StrictModel):
    ...
    Consumed by :class:`backend.src.robot.grasping.refinement.RefinementPolicy`
...
class GraspingVerificationConfig(StrictModel):
    ...
    :class:`backend.src.robot.grasping.verification.GraspVerificationPolicy`.
    ...
    width_delta_max_mm: float | None = Field(default=None, ge=0.0, le=50.0)
...
class GraspingUncertaintyConfig(StrictModel):
    ...
    ranking_penalty_weight: float = Field(
        ...
        json_schema_extra={
            ...
            "u11_min": 0.0,
            "u11_max": 1.0,
            "u11_max_abs_step": 0.20,
            "u11_max_rel_step": 0.50,
        },
    )
    recovery_aggressive_threshold: float = Field(
        ...
        json_schema_extra={
            ...
            "u11_min": 0.0,
            "u11_max": 1.01,
            "u11_max_abs_step": 0.10,
            "u11_max_rel_step": 0.30,
        },
    )
    ...
    # rerank_modes is dense-only and SEPARATE from apply_modes (which includes "auto" and would crash the
    # dense-only carrier); default off + weight 0.0 is byte-identical.
...
class GraspingSuccessModelConfig(StrictModel):
    ...
    ranking_blend_weight: float = Field(
        ...
        json_schema_extra={
            ...
            "u11_min": 0.0,
            "u11_max": 0.5,
            "u11_max_abs_step": 0.05,
            "u11_max_rel_step": 0.25,
        },
    )
...
class GraspingWatchdogConfig(StrictModel):
    ...
    block_modes
        ...
        :class:`backend.src.robot.grasping.modes.GraspMode`) that fall under the
...
class GraspingPerformanceConfig(StrictModel):
    ...
    :class:`backend.src.robot.grasping.latency_tracker.LatencyTracker` seam owned
...
class RobotGraspingCommitPolicyConfig(StrictModel):
    ...
    :attr:`~backend.src.robot.grasping.pick_loop.PickOutcome.NO_COMMIT_INSUFFICIENT_FUSION`.
...
class RobotGraspingFusionConfig(StrictModel):
    ...
    :class:`backend.src.robot.grasping.multi_view_fusion.SceneFusion` maintains
...
class RobotGraspingConfig(StrictModel):
    ...
    :class:`backend.src.robot.grasping.modes.GraspMode` at use time so the config

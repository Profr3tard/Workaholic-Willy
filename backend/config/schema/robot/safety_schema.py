...
class RobotSafetyConfig(StrictModel):
    ...
    * :attr:`dwell` -- post-Stop dwell + steady-state gating.
    ...
    A configuration key used to claim otherwise (``emergency_stop_source``, selecting a "transport" for
    the signal); no code ever read it, so it described a capability that did not exist. Stating the
    truth is worth more than a knob that only looks like safety.
    """

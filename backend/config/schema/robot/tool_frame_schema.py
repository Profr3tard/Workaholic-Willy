...
point sits relative to the robot's flange, and how it is turned, is a property of the MOUNTED TOOL, and
until this module existed it had no representation outside the sim driver.
"""
...
#:
#: Both live modes are checked by the same measurement, and they predict different answers -- ~identity
#: for ``willy``, the declared transform for ``polyscope`` -- so the check also proves which mode the
#: cell is really in, not merely that some number matched.
...
class ToolFrameConfig(StrictModel):
    ...
    **(3) NOT VALIDATED ON REAL HARDWARE.** Every number here must be MEASURED on the actual cell --
    ...
    while closing the jaws along the wrong object axis -- and hand-eye calibration cannot catch it,
